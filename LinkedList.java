...
public class LinkedList<T> implements List<T>
{
	...
	public void add(int position, T data)
	{
		...
		else
		{
			...
			for (int i = 0; i < position-1; i++) //up until desired position, data moves over to next position
	...
	public T get(int position) throws Exception //retrieving a value at a particular position
	{
		...
		for (int i = 0; i < position-1; i++)  //changing node until it reaches desired position
			node = node.next;
